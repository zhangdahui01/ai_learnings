...
| `test-suites/<plan-name>/<suite-name>/*.spec.js` | Online-editable Node.js/Playwright tests | One file per case, including Case Setup/Steps/Teardown. |
| `test-suites/<plan-name>/<suite-name>/test_*.py` | Python/Playwright generated from structured steps | Teardown uses `finally`; the app runs JavaScript, while Python can run separately. |
