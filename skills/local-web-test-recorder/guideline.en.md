...
| Path | Content | Notes |
|---|---|---|
| `data/store.json` | Test plans, cases, steps, assertions, test data, account references, browser/locale/proxy settings, and metadata for the latest 50 runs | The current release uses a local JSON file, not a remote database. Test data may be sensitive. |
...
- The application retains metadata for the latest 50 runs, but files under `artifacts/` are not automatically purged.
...
Use the same locale for recording and replay. The runner also adds a role-only fallback for role locators whose accessible names are localized.
...
Open a case and click Replay. The editor saves changes before the run. Version protection prevents an old browser tab from overwriting a newer imported case.

The current release replays one case at a time. Plans organize cases; full plan batch execution remains a future extension.
...
The suite covers codegen import, locator preservation, stale-version protection, locale fallback, waits, assertions, and Chromium replay.
