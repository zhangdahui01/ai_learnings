...
| `test-suites/<plan-name>/*.spec.js` | Online-editable, executable Node.js/Playwright tests | One file per case; suitable for version control after secret review. |
| `test-suites/<plan-name>/test_*.py` | Python/Playwright generated from structured steps | The app runs JavaScript; run Python separately after installing Python Playwright. |
...
- Standard-mode temporary profiles: normally created in the operating-system temporary directory and removed by Playwright after the browser closes.
- Compliant-mode profiles and login state: stored under `data/profiles/` and `data/auth/`; they are not removed when Chrome closes.
