...
| 路径 | 保存内容 | 注意事项 |
|---|---|---|
| `data/store.json` | 测试计划、用例、步骤、断言、测试数据、账号引用、浏览器/语言/代理配置、最近 50 次运行元数据 | 当前版本使用本地 JSON，不是远程数据库。测试数据可能敏感。 |
...
- 应用只保留最近 50 次运行的元数据，但 `artifacts/` 文件不会自动清理。
...
录制和回放应使用相同语言。工具也会为带本地化名称的 role 定位器增加语言无关回退。
...
打开用例后点击“回放”。回放前工具会保存编辑内容。版本保护会阻止旧浏览器页面覆盖更新后的导入结果。

当前版本支持单用例回放。测试计划用于组织用例；完整的计划批量执行仍属于后续扩展范围。
...
该测试覆盖 codegen 导入、定位器保留、旧版本覆盖保护、语言回退、等待、断言和 Chromium 回放。
