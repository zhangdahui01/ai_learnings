...
function structuredSources(db, name, steps, data = {}) {
  const expanded = expandSteps(db, steps || [], data, [], 'steps').map(item => item.step); const virtual = { ...defaultCase(name), data, steps: expanded };
...
async function persistFlowArtifacts(db, flow, regenerate = false) {
  if (regenerate || !flow.sources?.javascript || !flow.sources?.python) flow.sources = structuredSources(db, flow.name, flow.steps);
