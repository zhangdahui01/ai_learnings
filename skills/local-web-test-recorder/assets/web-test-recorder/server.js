...
const defaultCase = (name = '新测试用例') => ({
  ...
  defaults: { browser: 'chromium', baseUrl: '', locale: 'zh-CN', session: { mode: 'clean', persistRecordingState: false }, proxy: { mode: 'direct', server: '', username: '', password: '', bypass: '', mappings: [] }, stability: { preset: 'standard', navigationTimeoutMs: 30000, actionTimeoutMs: 10000, assertionTimeoutMs: 10000 }, timeoutMs: 10000 },
...
app.post('/api/suites', async (req, res) => { const db = await store(); const suite = { id: randomUUID(), name: String(req.body.name || '新测试套件'), description: String(req.body.description || ''), caseIds: [], setupSteps: [], teardownSteps: [], sessionPolicy: req.body.sessionPolicy || 'clean-per-case', createdAt: now(), updatedAt: now() }; db.suites.push(suite); if (req.body.planId) { const plan = db.plans.find(item => item.id === req.body.planId); if (plan && !plan.suiteIds.includes(suite.id)) plan.suiteIds.push(suite.id); } await save(db); res.status(201).json(suite); });
app.put('/api/suites/:id', async (req, res) => { const db = await store(); const index = db.suites.findIndex(item => item.id === req.params.id); if (index < 0) throw httpError(404, '未找到测试套件', 'SUITE_NOT_FOUND'); const suite = { ...db.suites[index], ...req.body, id: req.params.id, updatedAt: now() }; for (const key of ['caseIds','setupSteps','teardownSteps']) if (!Array.isArray(suite[key])) throw httpError(400, `${key} 必须是数组`, 'INVALID_SUITE'); db.suites[index] = suite; await save(db); res.json(suite); });
...
app.post('/api/suites/:id/cases/:caseId', async (req, res) => { const db = await store(); const suite = db.suites.find(item => item.id === req.params.id); const testCase = db.cases.find(item => item.id === req.params.caseId); if (!suite || !testCase) throw httpError(404, '未找到套件或用例', 'SUITE_OR_CASE_NOT_FOUND'); if (!suite.caseIds.includes(testCase.id)) suite.caseIds.push(testCase.id); suite.updatedAt = now(); await persistSources(db, testCase); await save(db); res.json(suite); });
...
app.post('/api/flows', async (req, res) => { const db = await store(); const flow = { id: randomUUID(), name: String(req.body.name || '新公共流程'), description: String(req.body.description || ''), parameters: Array.isArray(req.body.parameters) ? req.body.parameters : [], safety: req.body.safety || 'normal', steps: Array.isArray(req.body.steps) ? req.body.steps : [], version: 1, revisions: [], createdAt: now(), updatedAt: now() }; db.flows.push(flow); await save(db); res.status(201).json(flow); });
app.put('/api/flows/:id', async (req, res) => { const db = await store(); const index = db.flows.findIndex(item => item.id === req.params.id); if (index < 0) throw httpError(404, '未找到公共流程', 'FLOW_NOT_FOUND'); const old = db.flows[index]; const changed = JSON.stringify({ steps: old.steps, parameters: old.parameters, safety: old.safety }) !== JSON.stringify({ steps: req.body.steps ?? old.steps, parameters: req.body.parameters ?? old.parameters, safety: req.body.safety ?? old.safety }); const revisions = [...(old.revisions || [])]; if (changed) revisions.push({ version: old.version, steps: old.steps, parameters: old.parameters, safety: old.safety, savedAt: old.updatedAt }); const flow = { ...old, ...req.body, id: old.id, createdAt: old.createdAt, version: changed ? old.version + 1 : old.version, revisions, updatedAt: now() }; if (!Array.isArray(flow.steps) || !Array.isArray(flow.parameters)) throw httpError(400, '公共流程步骤和参数必须是数组', 'INVALID_FLOW'); db.flows[index] = flow; await save(db); res.json(flow); });
...
app.post('/api/flows/:id/run', async (req, res) => { const db = await store(); const flow = db.flows.find(item => item.id === req.params.id); if (!flow) throw httpError(404, '未找到公共流程', 'FLOW_NOT_FOUND'); const virtual = { ...defaultCase(`${flow.name} · 公共流程`), steps: flow.steps, data: req.body?.data || {} }; const result = await executeVisualCase(db, virtual, { ...(req.body || {}), scope: 'flow' }); result.flowId = flow.id; result.flowName = flow.name; await recordRun(result); res.status(result.status === 'passed' ? 200 : 422).json(result); });
...
async function autoImportRecording(session, overwrite = false) {
  ...
  const virtualCase = { ...defaultCase(target.label), steps }; const python = generatePythonAdvanced(virtualCase);
  if (session.targetType === 'suite') { target.owner[session.phase] = steps; target.owner.sources ||= { setupSteps: {}, teardownSteps: {} }; target.owner.sources[session.phase] = { javascript, python }; target.owner.updatedAt = now(); }
  else if (session.targetType === 'flow') { target.owner.revisions.push({ version: target.owner.version, steps: target.owner.steps, parameters: target.owner.parameters, safety: target.owner.safety, sources: target.owner.sources, savedAt: target.owner.updatedAt }); target.owner.version += 1; target.owner.steps = steps; target.owner.sources = { javascript, python }; target.owner.updatedAt = now(); }
...
async function startStructuredRecording({ targetType, targetId, phase = null, name, defaults }, body = {}) {
  ...
  const session = { id: sessionId, targetType, targetId, phase, status: 'waiting-for-user', dryRun: process.env.RECORD_DRY_RUN === '1', complianceMode: false, persistSession: false, startedAt: now(), outputPath: out, authPaths: null, debugArgs: args, outputFile: relative(root, out), message: `${name} 录制窗口已打开。完成后关闭 Inspector，代码和无代码步骤将自动导入。` }; recordingSessions.set(sessionId, session);
  if (process.env.RECORD_DRY_RUN !== '1') { const child = spawn('npx', args, { cwd: root, detached: true, stdio: 'ignore' }); session.pid = child.pid; child.once('error', error => completeRecordingSession(session, null, error)); child.once('exit', code => completeRecordingSession(session, code)); child.unref(); }
