...
async function store() {
  ...
  db.suites.forEach(item => { item.caseIds ||= []; item.setupSteps ||= []; item.teardownSteps ||= []; item.sessionPolicy ||= 'clean-per-case'; });
  db.flows.forEach(item => { item.steps ||= []; item.parameters ||= []; item.version ||= 1; item.revisions ||= []; item.safety ||= 'normal'; });
...
app.post('/api/cases/:id/record', async (req, res) => {
  ...
  if (req.body.url) args.push(req.body.url); const session = { id: sessionId, caseId: testCase.id, status: 'waiting-for-user', dryRun: process.env.RECORD_DRY_RUN === '1', complianceMode, persistSession, temporaryProfile, startedAt: now(), manualVerification: complianceMode, outputPath: out, authPaths: paths, debugArgs: args, outputFile: relative(root, out), profile: complianceMode ? (persistSession ? `data/profiles/${safeFile(testCase.id)}` : '临时隔离 Profile（结束后删除）') : null, loginState: complianceMode && persistSession ? `data/auth/${safeFile(testCase.id)}.json` : null, message: complianceMode ? `正式 Chrome 已打开，使用${persistSession ? '显式持久化' : '全新临时'}会话。手工完成登录或 CAPTCHA 后继续；完成后关闭 Inspector（无需 Save）。` : '录制窗口已打开，使用全新临时会话。完成后关闭 Inspector（无需 Save），脚本会自动导入代码编辑器。' };
