...
const server = spawn(process.execPath,['server.js'],{cwd:root,env:{...process.env,PORT:String(port),DATA_DIR:paths.data,RECORDINGS_DIR:paths.recordings,ARTIFACTS_DIR:paths.artifacts,TEST_SUITES_DIR:paths.suites},stdio:'pipe'});
...
try{
  ...
  const browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport:{width:1440,height:1000}});await page.goto(base);await page.getByRole('heading',{name:'今天的测试状态'}).waitFor();assert.equal(await page.locator('.metric').count(),4);await page.locator('[data-view="cases"]').click();await page.locator(`[data-case="${testCase.id}"]`).click();await page.getByRole('button',{name:'无代码步骤'}).waitFor();assert.equal(await page.locator('#steps .step').count(),5);await page.getByRole('button',{name:'代码编辑'}).click();await page.locator('#codeEditor').waitFor();await page.locator('[data-view="runs"]').click();assert.ok(await page.locator('.run-row').count()>=4);await browser.close();

  await request(`/api/runs/${failed.id}`,'DELETE');assert.ok(!(await request('/api/runs')).some(x=>x.id===failed.id));
  console.log('E2E: plan folders, JS/Python, import, visual/code replay, diagnostics, artifacts, dashboard, and run CRUD passed');
