...
try{
  ...
  const browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport:{width:1440,height:1000}});await page.goto(base);await page.getByRole('heading',{name:'今天的测试状态'}).waitFor();assert.equal(await page.locator('.metric').count(),4);await page.locator('[data-view="cases"]').click();await page.locator(`[data-case="${testCase.id}"]`).click();await page.getByRole('button',{name:'无代码步骤'}).waitFor();assert.equal(await page.locator('#steps .step').count(),5);await page.getByRole('button',{name:'配置与数据'}).click();assert.equal(await page.locator('.compliance-card').count(),0);
  ...
  console.log('E2E: plans, JS/Python, automatic recording import, replay, diagnostics, artifacts, dashboard, compliant Chrome state, and run CRUD passed');
