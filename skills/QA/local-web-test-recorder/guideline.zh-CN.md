...
进入“Repo 知识图谱”可为开发 Repo、组件/Page Object Repo、既有自动化框架分别构建图谱。推荐先运行 Graphify；未安装时可使用内置代码图。生成弹窗中选择平台内的 Test Plan 和 Test Suite，同时多选只读参考图谱；图谱不会成为脚本写入位置。首次生成会在该 Suite 下创建 BDD 生成 Test Case；重复生成可选择覆盖同一 Case，系统会创建新版本并保留旧脚本与回放记录。知识图谱和 QA 批准后的 BDD 都是必需项，Codegen 是可选证据。页面自动识别 Codex、Claude Code、Devin 或普通本地 Agent。
...
2. 在“Repo 知识图谱”为每个需要参考的本地 Repo 构建或刷新 READY 图谱。创建任务时选择目标 Test Plan 与 Test Suite，并多选参考图谱；若同一 BDD 已生成过 Case，可选择覆盖并创建新版本。
...
- `data/generation-jobs/<job-id>/` 与 `test-suites/_bdd-generation/<job-id>/`：冻结 BDD、生成过程和待验证脚本；验证后同步到所选 Test Plan → Test Suite → Test Case 的版本目录。
...
4. QA 批准 BDD 后，在生成弹窗选择目标 Test Plan、Test Suite 和多个参考图谱；重复任务可选择覆盖现有 BDD 生成 Case。Devin 对话中执行：`@skills:local-web-test-recorder 处理 queued Playwright 生成任务，回放并修复直到通过或达到上限`。
...
| `test-suites/.../versions/vN/native.<phase>.spec.js` | vN 对应阶段可编辑、可直接回放的原生 JavaScript | 在“高级功能”保存会创建新资产版本；不会修改 `recorded.<phase>.spec.js` 或无代码步骤。 |
...
### 录制快照、原生回放代码、无代码步骤和导出代码

- 录制时原始快照：Inspector 关闭后按资产、阶段和版本永久保存，只读，用于审计、恢复和对照，任何编辑都不会覆盖它。
- 原生回放 JavaScript：初始内容来自录制快照，可在“高级功能”直接编辑。例如使用“手工登录后录制”却未正确截断时，可删除登录完成之前的语句。保存后创建新版本，只更新当前阶段的原生回放代码，并把该阶段默认回放方式设为原生代码。
- 无代码步骤：导入器尽力从原始录制生成，供小白查看、修改、增加等待和断言。保存后只更新结构化步骤，并重建对应的 JS/Python 导出。
- 导出 JavaScript/Python：由无代码步骤生成，用于本地命令执行、审阅和二次开发；保持只读，不会反向修改无代码步骤、原生回放代码或录制快照。
- 普通页面不再显示容易混淆的“代码编辑”标签。进入“高级功能”可编辑原生回放 JavaScript、查看只读录制快照与只读导出。平台不再提供“复制为自定义代码用例”，避免误创建额外的空用例；已有历史自定义代码用例继续兼容编辑。
- 打开“版本历史”后，每个版本按 Case Setup、测试步骤、Case Teardown（或对应 Suite/公共流程阶段）展示默认回放方式。`原生代码` 表示默认直接执行该版本的 `native.<phase>.spec.js`；`无代码步骤` 表示默认执行结构化步骤；`无代码步骤（可切换原生）` 表示默认是无代码，但回放弹窗仍可手工选择原生代码。
- 历史用例没有原始录制字段时继续按原来的无代码步骤运行；只有 JavaScript、完全没有结构化步骤的旧代码用例会兼容执行现有 JavaScript。迁移不会把旧的生成代码冒充为原始录制。

界面只让用户在两种“执行来源”之间选择：**按原生代码回放**或**按当前步骤回放**，不会展示三个容易混淆的产品版本。录制快照只是原始证据，JS/Python 导出只是无代码版本的文件表达，两者都不是额外的业务版本。修改无代码步骤不会主动覆盖原生回放代码。
...
- 单阶段、用例和公共流程回放：选择 Stable/Latest/指定版本后，再选“按原生代码回放”或“按当前步骤回放”。没有原生回放代码的历史版本会明确提示并回退到当前步骤。
- Suite/计划执行：选择“各阶段默认”“优先原生代码”或“全部当前步骤”。Suite Setup、公共流程、Case Setup/主体/Teardown 和 Suite Teardown 可以混用两种引擎，但始终共享同一个 Browser/Context/Page、一段录像和一个 Trace。
...
版本选择之后还要选择执行来源：“按原生代码回放”执行该版本可编辑的 Playwright 原生回放代码；“按当前步骤回放”执行该版本结构化步骤。Suite/Plan 可以统一选择各阶段默认、优先原生代码或全部当前步骤，执行记录会冻结每个阶段实际采用的来源。
