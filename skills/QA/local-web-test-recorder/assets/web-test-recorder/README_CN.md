...
- 把 Excel 按 `Depth1 + Depth2 + Depth3 + Pre-requisite` 聚合为 BDD；同组 Step/Expected Result 始终成对保存。
- 把 QA 批准的 BDD 与多个可选 Repo 知识图谱交给当前 AI Agent，单独指定一个输出 Repo 生成 Playwright 脚本，并按“生成 → 回放 → Agent 修复 → 再回放 → QA 签署”闭环验收。
