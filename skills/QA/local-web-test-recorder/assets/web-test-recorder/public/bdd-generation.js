...
export function openGenerationDialog({ item, knowledge, plans = [], suites = [], testCases = [], runtime, api, esc, toast, refresh, lines, openKnowledgeManager }) {
  ...
  let availablePlans = [...plans]; let availableSuites = [...suites];
  const usablePlans = () => availablePlans;
  ...
  node.innerHTML = `<div class="modal bdd-generation-modal">
    ...
    <div class="field-grid"><div class="field"><div class="split"><label>生成后归属 Test Plan（必需）</label><button type="button" class="btn tiny secondary" id="createGenerationPlan">+ 新建 Plan</button></div><select id="generationPlan"><option value="">${usablePlans().length ? '请选择 Test Plan' : '暂无 Test Plan，请先新建'}</option>${usablePlans().map(plan => `<option value="${esc(plan.id)}">${esc(plan.name)}</option>`).join('')}</select></div><div class="field"><div class="split"><label>生成后归属 Test Suite（必需）</label><button type="button" class="btn tiny secondary" id="createGenerationSuite" disabled>+ 新建 Suite</button></div><select id="generationSuite" disabled><option value="">请先选择 Test Plan</option></select></div></div>
  ...
  const planSelect = node.querySelector('#generationPlan'); const suiteSelect = node.querySelector('#generationSuite'); const caseSelect = node.querySelector('#generationExistingCase'); const createPlanButton = node.querySelector('#createGenerationPlan'); const createSuiteButton = node.querySelector('#createGenerationSuite');
  const syncPlans = (selected = '') => { planSelect.innerHTML = `<option value="">${usablePlans().length ? '请选择 Test Plan' : '暂无 Test Plan，请先新建'}</option>${usablePlans().map(plan => `<option value="${esc(plan.id)}" ${plan.id === selected ? 'selected' : ''}>${esc(plan.name)}</option>`).join('')}`; };
  ...
  const syncSuites = (selected = '') => { const planId = planSelect.value; const choices = availableSuites.filter(suite => suite.planId === planId); suiteSelect.disabled = !choices.length; createSuiteButton.disabled = !planId; suiteSelect.innerHTML = choices.length ? `<option value="">请选择 Test Suite</option>${choices.map(suite => `<option value="${esc(suite.id)}" ${suite.id === selected ? 'selected' : ''}>${esc(suite.name)}</option>`).join('')}` : '<option value="">请选择或新建 Test Suite</option>'; syncCases(); };
  createPlanButton.onclick = async () => { const name = window.prompt('Test Plan 名称'); if (!name?.trim()) return; try { const plan = await api('/api/plans', { method: 'POST', body: JSON.stringify({ name: name.trim() }) }); availablePlans.push(plan); syncPlans(plan.id); syncSuites(); toast(`已创建 Test Plan：${plan.name}`); } catch (error) { toast(error.message, 'error'); } };
  createSuiteButton.onclick = async () => { const planId = planSelect.value; if (!planId) return toast('请先选择 Test Plan', 'error'); const name = window.prompt('Test Suite 名称'); if (!name?.trim()) return; try { const suite = await api('/api/suites', { method: 'POST', body: JSON.stringify({ planId, name: name.trim() }) }); availableSuites.push(suite); syncSuites(suite.id); toast(`已创建 Test Suite：${suite.name}`); } catch (error) { toast(error.message, 'error'); } };
